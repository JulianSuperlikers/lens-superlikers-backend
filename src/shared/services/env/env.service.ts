...
export class EnvService {
  ...
  getConfig(campaignId: string): Config {
    try {
      const campaingConfig = getMicrositeConfig(campaignId);
      const campaign = campaingConfig.name;
      ...
      throw new InternalServerErrorException('Error getting environment variables');
    }
  }
